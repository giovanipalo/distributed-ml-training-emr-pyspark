...
def treina_avalia_modelo(spark, classifier, features, classes, train, test, bucket, nome_bucket, ambiente_execucao_EMR):
    ...
    def find_mtype(classifier):
        M = classifier
        Mtype = type(M).__name__
        return Mtype
    
    # Cria instância da classe
    Mtype = find_mtype(classifier)
    
    # Método para o treinamento do modelo
    def instancia_fit_model(Mtype, classifier, classes, features, train):
    ...
    fitModel = instancia_fit_model(Mtype, classifier, classes, features, train)
...
def cria_modelos_ml(spark, HTFfeaturizedData, TFIDFfeaturizedData, W2VfeaturizedData, bucket, nome_bucket, ambiente_execucao_EMR):
    ...
    for featureDF in featureDF_list:
        ...
        for classifier in classifiers:
            ...
            new_result = treina_avalia_modelo(spark,
                                               classifier,
                                               features,
                                               classes,
                                               train,
